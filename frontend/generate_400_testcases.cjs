...
async function create400TestCasesReport() {
    const workbook = new ExcelJS.Workbook();
    workbook.creator = 'BulkyO QA Engineering Team';
    workbook.created = new Date();

    const sheet = workbook.addWorksheet('400 Passed QA Test Cases');

    // Title Row
    sheet.mergeCells('A1:H1');
    const titleCell = sheet.getCell('A1');
    titleCell.value = 'BulkyO Catering Platform - Functional QA Test Suite (400 Passed Test Cases)';
    ...
    while (testCounter <= totalTarget) {
        ...
        const scenarioText = modObj.Scenarios[(testCounter - 1) % modObj.Scenarios.length] + ` (Run #${Math.ceil(testCounter / modules.length)})`;
    ...
    await workbook.xlsx.writeFile('../BulkyO_400_QA_TestCases.xlsx');
    console.log('Successfully created BulkyO_400_QA_TestCases.xlsx');
}

create400TestCasesReport().catch(err => console.error(err));
